# Resources
This page lists all resources available for JSONLoader which may be of use to you!

## Built-In
* https://tinyurl.com/JLDRCards2 <-- Cards (extension: `.jdlr2`)
* https://tinyurl.com/JLDREncounters <-- Encounters (extension: `_encounter.jldr2`)
* https://tinyurl.com/JLDRGramophoneTracks <-- Gramophone Tracks (`_gram.jldr2`)
* https://tinyurl.com/JLDRLanguages <-- Languages (extension: `_language.jldr2`)
* https://tinyurl.com/JLDRMasks <-- Masks (extension: `_mask.jldr2`)
* https://tinyurl.com/JLDRSigils <-- Sigils (extension: `_sigil.jldr2`)
* https://tinyurl.com/JLDRStarterDecks2 <-- Starter Decks (extension: `_deck.jldr2`)
* https://tinyurl.com/JLDRTalkingCards <-- Talking Cards (extension: `_talk.jldr2`)
* https://tinyurl.com/JLDRTribes <-- Tribes (extension: `_tribe.jldr2`)

## Extension Mods:
* https://tinyurl.com/JLDRAnimatedPortraits2 <-- Animated Portraits (extension: `.json`)
* https://tinyurl.com/JLDRBossCards2 <-- Boss Cards (extension: `_giant.json`)
* https://tinyurl.com/JLDRBossDialouge2 <-- Boss Dialouge (extension: `_bd.json`)
* https://tinyurl.com/JLDRCardPacks2 <-- Card Packs (extension: `.jlpk`)

## Extra Stuff
* All Enums (Including mods): <https://github.com/SaxbyMod/SabyModEnums/wiki>
* Configils Wiki: <https://docs.google.com/document/d/1QLAfomaTcatm-foU2P1ZoqGQFFvhCfmEnN4jIxAWceQ/edit?usp=sharing>
* Official JSONLoader Wiki: <https://github.com/MADH95/JSONLoader/wiki>
